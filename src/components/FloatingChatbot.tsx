...
export function FloatingChatbot() {
  ...
  // Check tenant RAG status on mount and when session changes
  useEffect(() => {
    const checkRagStatus = async () => {
      if (session?.user?.tenant_id) {
        try {
          ...
          console.log(`✅ Tenant RAG status checked: ${isRag ? 'enabled' : 'disabled'}`)
        } catch (error) {
          console.warn("⚠️ Could not check tenant RAG status, defaulting to false:", error)
          setTenantRagEnabled(false)
        }
      } else {
        setTenantRagEnabled(null)
      }
    }
                      ...
                      >
                        {message.type === "user" ? (
                          ...
                          <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-2xl rounded-br-md px-3 py-2 shadow-sm">
                            <p className="text-xs font-medium leading-relaxed break-words">{message.content}</p>
                            ...
                            {message.content.trim() && (
                              <div className="bg-white dark:bg-card border border-gray-100 dark:border-border rounded-2xl rounded-tl-md px-3 py-2 shadow-sm">
                                <div className="flex items-start justify-between gap-2">
                                  <p className="text-xs text-gray-800 dark:text-card-foreground leading-relaxed whitespace-pre-wrap flex-1 min-w-0 break-words">
                                    {message.content}
                                  </p>
                                  <button
